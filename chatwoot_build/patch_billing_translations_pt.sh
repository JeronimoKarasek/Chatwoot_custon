#!/bin/sh
# Patch PT_BR settings.json to add FarolChat billing keys (Asaas)
node -e "
const fs = require('fs');
...
if (data.BILLING_SETTINGS) {
  data.BILLING_SETTINGS.FAROLCHAT = {
    ...
    MANAGE_DESCRIPTION: 'Ajuste a quantidade de usuários e conexões da sua conta. As alterações são sincronizadas automaticamente.',
...
console.log('[patch] PT_BR settings.json billing keys patched (Asaas)');
"
