class Api::V1::Accounts::Evolution::AuthorizationsController < Api::V1::Accounts::BaseController
  ...
  def configure_instance_settings(api_url, api_hash, instance_name, behavior_settings)
    # Configure settings via Evolution API settings endpoint
    ...
    # Don't fail if settings endpoint doesn't work - it may not exist in all versions
    ...
    # Don't raise - settings are optional
  end
  ...
  def evolution_webhook_url(instance_name)
    # Use configured host or request host
  ...
  def status
    ...
    # Use hardcoded values as fallback since ENV may not be set
    api_url = ENV['EVOLUTION_API_URL'].presence || 'https://evochat4.farolchat.com'
    admin_token = ENV['EVOLUTION_ADMIN_TOKEN'].presence || 'EVOLUTION_TOKEN_REDACTED'
  ...
  def create_chatwoot_inbox(inbox_name, phone_number, instance_name, instance_data, api_url, admin_token)
    Rails.logger.info "Creating Chatwoot inbox: #{inbox_name} for instance: #{instance_name}"

    # Format phone number to ensure it starts with +
    formatted_phone = phone_number.start_with?('+') ? phone_number : "+#{phone_number}"
    # Make phone number unique by adding timestamp if needed
    unique_phone = formatted_phone
    
    # Check if phone number already exists
    existing_channel = Channel::Whatsapp.find_by(phone_number: formatted_phone)
    if existing_channel
      # Use existing channel's inbox if it exists
      existing_inbox = existing_channel.inbox
      if existing_inbox && existing_inbox.account_id == current_account.id
        Rails.logger.info "Inbox already exists for this phone number, updating config"
        # Update the provider_config with Evolution data
        ...
        # Phone already used by another account, make it unique
        unique_phone = "#{formatted_phone}_#{Time.now.to_i}"
        Rails.logger.info "Phone number exists, using unique: #{unique_phone}"
      end
    end

    # Create the WhatsApp channel with Evolution provider
    # Using provider: 'default' with api_key to pass validation
    channel = Channel::Whatsapp.new(
      account: current_account,
      phone_number: unique_phone,
      provider: 'default',
      provider_config: {
        'api_key' => 'evolution_api_placeholder', # Required for validation
        'api_url' => api_url,
        'admin_token' => admin_token,
        'instance_name' => instance_name,
        'api_hash' => instance_data['hash'],
    ...
    Rails.logger.info "Created WhatsApp channel with ID: #{channel.id}"
    ...
    # The Evolution instance was already created
    nil
  end
end
