...
/**
 * FarolChat: Modified to extract customName from rest params and pass it
 * to createNewContact so new contacts get the proper name.
 */
const handleSelectedContact = async ({ value, action, ...rest }) => {
  ...
  if (action === 'create') {
    ...
    try {
      const customName = rest.customName || null;
      contact = await createNewContact(value, customName);
      ...
      const errorMsg =
        ...
        error?.message ||
        t('COMPOSE_NEW_CONVERSATION.CONTACT_SEARCH.ERROR_MESSAGE');
      useAlert(errorMsg);
      return;
    }
  } else {
